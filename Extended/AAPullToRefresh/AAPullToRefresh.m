...
- (void)setupScrollViewContentInsetForLoadingIndicator:(actionHandler)handler animated:(BOOL)animated
{
    ...
    [self setScrollViewContentInset:currentInsets handler:handler animated:animated];
}

- (void)resetScrollViewContentInset:(actionHandler)handler
{
	[self resetScrollViewContentInset:handler animated:YES];
}
- (void)resetScrollViewContentInset:(actionHandler)handler animated:(BOOL)animated {
	UIEdgeInsets currentInsets = self.scrollView.contentInset;
...
- (void)actionTriggeredState
{
    ...
    [self setupScrollViewContentInsetForLoadingIndicator:nil animated:YES];
    if (self.pullToRefreshHandler)
        self.pullToRefreshHandler(self);
}

- (void)actionStopStateAnimated:(BOOL)isAnimated
{
	if (isAnimated) {
...
- (void)stopIndicatorAnimationAnimated:(BOOL)isAnimated {
    [self actionStopStateAnimated:isAnimated];
}
- (void)stopIndicatorAnimation
{
    [self actionStopStateAnimated:YES];
}
