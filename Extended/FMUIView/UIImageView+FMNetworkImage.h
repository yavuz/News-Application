...
@property (nonatomic, assign) BOOL fixImageCropResize;
@property (nonatomic, assign) BOOL cacheDecodedResults;		// performance in exchange of (easily disposable) memory usage
// forceDelayEvenIfCached is temporary, see the TODO note on implementation
@property (nonatomic, assign) BOOL forceDelayEvenIfCached;	// makes sure you don't put images, when fast scrolling through them
@property (nonatomic, assign) NSTimeInterval delayBeforeLoading;
@property (nonatomic, copy) UIImage* (^onLoad) (UIImage* image);
