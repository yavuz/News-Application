...
@property (nonatomic,readwrite) CGSize imageViewPixelSize;
@property (atomic,retain) FMNetworkImageConnectionDelegate* connectionDelegate;
...
+ (void)attachTo:(id<FMNetworkImageAttachable>)object {
	...
	if ([object isKindOfClass:UIImageView.class])
		object.netImage.imageViewPixelSize = (CGSize){ [(UIImageView*)object bounds].size.width, [(UIImageView*)object bounds].size.height };
}
...
- (void)setURL:(NSURL *)URL completionBlock:(UIImage* (^)(UIImage * image))onLoad {
	self.onLoad = onLoad;
	self.URL = URL;
}
...
- (void)doneLoadingImage:(UIImage*)image error:(NSError*)error {	// this should be called on main thread
	...
	if (_completetionTarget && _completetionSelector)
		image = [_completetionTarget performSelector:_completetionSelector withObject:image withObject:self];
	else if (_onLoad)
		image = _onLoad(image);
...
- (BOOL)tryCached:(NSURL*)url {
	[self retain];
	UIImage* cached = [self.class cachedImageForURL:url];
	if (cached && ![FMNetworkImageOperation.currentOperation isCancelled]) {
...
- (void)realInitiateWithURL:(NSURL*)url {
	...
	if (!_forceDelayEvenIfCached) {
		if ([self tryCached:url])
			return;
	}
	...
	if (_forceDelayEvenIfCached) { // now you can try
		if ([self tryCached:url])
			return;
	}
	...
	self.connectionDelegate = [[FMNetworkImageConnectionDelegate.alloc initWithFMNetworkImage:self] autorelease];
	_connectionDelegate.conn = [NSURLConnection.alloc initWithRequest:aRequest delegate:_connectionDelegate startImmediately:NO].autorelease;
	
	[_connectionDelegate.conn setDelegateQueue:self.queue];
	[_connectionDelegate.conn start];
}
- (void)cancelLoading {
...
- (void)decodeImage {
	...
	receiverSize.width *= _imageScale;
	receiverSize.height *= _imageScale;
	...
	if (!_queue) {
		@synchronized (self) {
			if (/*still*/ !_queue) {
				...
				_queue.name = @"FMNetworkImageSeparateQueue";
			}
		}
	}
	return _queue;
